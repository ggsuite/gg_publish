...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_publish.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggPublish = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_publish.dart',
      );

      expect(
        ggPublish,
        isNotNull,
        reason: 'Item for executing bin/gg_publish.dart not found',
      );

      expect(ggPublish['name'], 'gg_publish.dart');
      expect(ggPublish['type'], 'dart');
      expect(ggPublish['request'], 'launch');
